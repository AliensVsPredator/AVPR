package org.avpr.common.entities.alien.draco_line;
...
/**
 * The OvamorphDracoEntity class represents an entity in the game that simulates an ovamorph (egg) from the Alien
 * franchise. This entity is responsible for hatching and potentially spawning a Facehugger entity.
 */
public class OvamorphDracoEntity extends AlienEntity {

    private static final EntityDataAccessor<Boolean> IS_HATCHING = SynchedEntityData.defineId(
        OvamorphDracoEntity.class,
    ...
    public OvamorphDracoEntity(EntityType<? extends AlienEntity> entityType, Level level) {
        super(entityType, level);
        this.vibrationUser = new AzureVibrationUser(this, 0.0F, 0);
    }

    /**
     * Creates and returns a builder for the attributes of the `OvamorphDracoEntity`. This builder initializes the
     * following attributes for the entity: - Maximum Health - Armor - Armor Toughness - Knockback Resistance - Follow
     * Range - Movement Speed
     *
     * @return A builder object for defining the attributes of the `OvamorphDracoEntity`.
     */
    public static AttributeSupplier.Builder createAttributes() {
        return Mob.createMobAttributes()
            .add(
                Attributes.MAX_HEALTH,
                CommonMod.config.ovamorphDracoConfigs.OVAMORPHDRACO_HEALTH
            )
            .add(
                Attributes.ARMOR,
                1.0
    ...
    /**
     * Registers the animation controllers for the `OvamorphDracoEntity`. This method defines the behavior of the
     * animation based on the state of the entity, such as whether it is hatching or already hatched.
     *
    ...
    /**
     * Handles the per-tick behavior of the `OvamorphDracoEntity`. This includes managing the hatching process, playing
     * specific sounds when hatching, and spawning a facehugger once hatched. The method extends its parent `tick`
     * method and introduces additional logic specific to the `OvamorphDracoEntity`: - Increments hatch progress if the
    ...
    /**
     * Handles the base tick behavior of the `OvamorphDracoEntity`. This method manages the following tasks: -
     * Increments the hatch check timer on each tick. - Initiates the hatching process if the entity has recently hurt
     * another mob. - Performs hatching checks every second (20 ticks), analyzing nearby entities to determine if
     * hatching conditions are met. - Checks blocks in the immediate vicinity and modifies the hatching state based on
     * their properties.
     */
    @Override
    public void baseTick() {
        ...
        if (this.hatchCheckTimer >= 20) {
            ...
            this.level()
                .getEntitiesOfClass(
                    LivingEntity.class,
                    this.getBoundingBox().inflate(CommonMod.config.ovamorphDracoConfigs.OVAMORPHDRACO_HATCH_RANGE)
                )
                .forEach(target -> {
